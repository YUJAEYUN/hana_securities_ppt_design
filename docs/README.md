...
이 디렉터리는 `hana-ppt-skill`의 요구사항, 현재 상태와 구현 순서를 관리한다. 실제 스킬 파일은 저장소 루트의 `hana-ppt-skill/`에 있으며, 최신 진행 상태는 [STATUS.md](STATUS.md)를 정본으로 확인한다.
...
- 승인된 브랜드 값은 `assets/brand.json` 한 곳에서 관리한다. 현재는 검수 전인 `assets/brand.candidate.json`만 존재한다.
...
단계의 실제 완료 여부는 `project-state.json`에서 관리하고 하네스가 [STATUS.md](STATUS.md)를 생성한다. 아래 설계 문서의 완료 조건은 목표이며 현재 완료를 뜻하지 않는다.
...
- 하나증권 리서치센터 2026년 8월 주식시장 전망 자료 URL — 사용자 판단에 따라 예외 자료로 분류했으며 스타일 학습에서는 제외
...
하나2.0의 정확한 패밀리명, 스타일과 임베딩 권한은 `hana-ppt-skill/assets/fonts/manifest.json`에 기록됐다. 색상은 HFG IR 자료에서 추정한 후보이며 하나증권 공식 색상으로 승인되지 않았다. CI 원본은 있지만 PPTX용 변환 파일과 보호영역은 아직 확정되지 않았다. 향후 생성기는 사용된 글리프만 PPTX에 서브셋으로 임베딩하고 PowerPoint 재열기와 폰트 미설치 렌더 결과까지 검증해야 한다.

## 반복 작업 하네스

파일 변경 후 다음 명령 하나로 상태 문서 생성, 검증, 커밋과 푸시를 수행한다.
...
현재 상태만 검증할 때는 `check`, `docs/STATUS.md`만 다시 만들 때는 `sync`를 사용한다. 실행 환경에 Python 또는 Git이 없으면 하네스는 해당 단계를 성공으로 가장하지 않고 중단한다.
