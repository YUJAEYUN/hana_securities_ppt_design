...
- 요약: 로컬 HFG IR 2건을 사용자 승인 운영 기준으로 삼아 출처·한계를 명시한 brand.json과 voice.json을 확정했다. 하나증권 공식 CI·공식 문체로 주장하지 않는다. PPT 처리 계층을 엔진(범용 PPTX 조작)과 규칙(하나증권 브랜드·문체) 두 겹으로 분리하기로 하고, LibreOffice+poppler 기반 render_slides.py에 이어 restyle_deck.py(테마 색상·폰트 적용, restyle-only)를 추가했다. 슬라이드 요소·레이아웃 수준 후처리, build_deck.py, 구조/시각 품질 검사 구현은 남아 있다.
...
| PPT 인수·후처리·생성 | 진행 중 | `hana-ppt-skill/scripts/ingest_deck.py`<br>`hana-ppt-skill/scripts/restyle_deck.py`<br>`hana-ppt-skill/schemas/deck_spec.schema.json`<br>`tests/test_repository.py` | restyle_deck.py의 슬라이드 요소·레이아웃 수준 재구성(현재는 테마 색상·폰트만 적용)<br>restyle_deck.py의 hana-refine 모드<br>build_deck.py<br>차트·SmartArt 상세 인수<br>실제 PPTX 회귀 픽스처 |
...
1. restyle_deck.py를 슬라이드 요소·레이아웃 수준(표지, 목차, 요약, 비교, 타임라인, 차트, 표)까지 확장하고, 원문·데이터 잠금 검사를 추가한다.
2. restyle_deck.py의 hana-refine 모드(문체·구조 개선, 출처 범위 안에서만)를 구현한다.
