...
- `restyle_deck.py` 1차 구현(완료): 테마 파트(`ppt/theme/themeN.xml`)의 색상·폰트만 승인된 `brand.json`으로 교체. 슬라이드 XML은 전혀 읽거나 쓰지 않아 `restyle-only`의 원문 잠금을 구조적으로 보장합니다. `hana-refine`은 아직 `NotImplementedError`로 명시적으로 막아 둡니다.
- 남은 범위: 표지, 목차, 요약, 비교, 타임라인, 차트, 표 등 슬라이드 요소·레이아웃 수준 재구성
